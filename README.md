...
universal (Apple Silicon + Intel) macOS 15+ DMG, a Windows MSI installer and a
...
retain the platform window controls and use macOS Vibrancy or Windows Mica;
unsupported compositors fall back without extra work.

Public Windows builds are fail-closed: configure `WINDOWS_CERTIFICATE_BASE64`
(a base64-encoded code-signing PFX) and `WINDOWS_CERTIFICATE_PASSWORD` as GitHub
Actions secrets. CI imports it only for the build, and Tauri signs the MSI and
executable with SHA-256 and a trusted timestamp. This prevents official releases
from silently publishing anonymous executables, which lack publisher reputation
and are much more likely to be quarantined by Defender or SmartScreen.
